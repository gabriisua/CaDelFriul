...
**Updated:** 2026-08-04 — after quick task 260804-kx5
...
| 260804-kx5 | Implement the Customer Dashboard feature — Spring Boot `GET /api/customers/me/dashboard` (JWT-principal-scoped, zero URL customerId): `CustomerDashboardResponse` DTO (upcomingReservation via existing `RoomReservationResponseDTO` + activeOrdersCount + savedAddressesCount), derived queries `findFirstByUserIdAndStatusAndCheckInDateGreaterThanEqualOrderByCheckInDateAsc` (nearest CONFIRMED, checkInDate ≥ today) + `countByCustomerIdAndStatusNotIn([CANCELLED, DELIVERED])`, address count reuses `AddressRepository.countByCustomerId`; frontend `CustomerDashboardResponse` interface + `fetchCustomerDashboard()` in API client; dashboard page renders live values in Upcoming Reservation / Active Orders / Saved Addresses cards with `<Skeleton className="h-4 w-32" />` while loading and placeholder text as error fallback; profile greeting + all links unchanged; zero new deps; gradle test + tsc + next build green | 2026-08-04 | `07faf20`, `a332dcf`, `6f3e6f9` | [260804-kx5-implement-the-customer-dashboard-feature](./quick/260804-kx5-implement-the-customer-dashboard-feature/) |
...
**Last activity:** 2026-08-04 — Completed quick task 260804-kx5: Dashboard now shows live data via new Spring Boot `GET /api/customers/me/dashboard` (be-cadelfriul): `CustomerDashboardResponse` (upcomingReservation via existing `RoomReservationResponseDTO`, activeOrdersCount, savedAddressesCount); reservations keyed by JWT principal email (`RoomReservation.userId` stores email — `findFirstByUserIdAndStatusAndCheckInDateGreaterThanEqualOrderByCheckInDateAsc` = nearest CONFIRMED, checkInDate ≥ today), orders keyed by Customer UUID (`countByCustomerIdAndStatusNotIn([CANCELLED, DELIVERED])`), addresses via existing `countByCustomerId`. Frontend: `CustomerDashboardResponse` + `fetchCustomerDashboard()` added to src/lib/api.ts (apiFetch, Bearer JWT, 401/403 → /login); dashboard page rewritten as dual-useEffect (profile greeting kept) with cancelled-guard dashboard fetch, `<Skeleton className="h-4 w-32" />` in the three dynamic cards while loading, placeholder text ("No upcoming reservations" / "No active orders" / "0 addresses saved") as error fallback; all card links unchanged. Zero new deps; `./gradlew compileJava` + `./gradlew test`, `npx tsc --noEmit`, `npm run build` all green
